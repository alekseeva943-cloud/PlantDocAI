...
async def update_status_loop(message, base_text: str, stop_event: asyncio.Event):
    """Анимация точек в сообщении (цикл . .. ...)."""
    dots = [".", "..", "..."]
    idx = 0
    while not stop_event.is_set():
        try:
            await message.edit_text(f"{base_text}{dots[idx % 3]}")
            idx += 1
            await asyncio.sleep(1)  # Немного быстрее для интерактивности
        except Exception:
            break
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_task = asyncio.create_task(
        update_status_loop(wait_msg, "🧐 Обрабатываю", stop_event))
    ...
    if not result:
        await wait_msg.edit_text("❌ Извините, произошла ошибка. Попробуйте позже.")
        return
    ...
    # Динамические кнопки (подсказки) - одноразовые
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE, provided_text: str = None):
    ...
    # Обработка сервисных слов
    if user_text in ["🌱 Новое растение", "/start"]:
        return await start(update, context)
    if user_text in ["🗑️ Очистить чат", "/clear"]:
        return await clear_chat(update, context)

    wait_msg = await update.message.reply_text("💬 Думаю")
    stop_event = asyncio.Event()
    status_task = asyncio.create_task(
        update_status_loop(wait_msg, "💬 Думаю", stop_event))
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stop_event = asyncio.Event()
    status_task = asyncio.create_task(
        update_status_loop(wait_msg, "🎤 Слушаю", stop_event))
...
if __name__ == '__main__':
    if not TELEGRAM_TOKEN or not OPENAI_API_KEY:
        ...
        # Регистрация команд в меню (на русском)
        commands = [
            BotCommand("start", "Start"),
            BotCommand("clear", "Начать новый анализ")
        ]

        async def post_init(application):
            # Установка команд для всех пользователей
            await application.bot.set_my_commands(commands)
            logging.info("Telegram commands menu initialized in Russian")
