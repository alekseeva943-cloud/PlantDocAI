...
Это должны быть вопросы, которые ПОЛЬЗОВАТЕЛЬ может задать в ответ, нажав на кнопку. 
Пишите их от первого лица пользователя. 
Примеры: "Какие гербициды мне использовать?", "Как часто мне его поливать?", "Что мне делать прямо сейчас?", "Это опасно для других растений?".
...
Верните ответ СТРОГО в формате JSON:
{
  ...
  "suggested_actions": ["Вопрос от пользователя 1", "Вопрос от пользователя 2"],
...
ВАЖНО ПОДСКАЗКИ (suggested_actions):
Это должны быть уточняющие вопросы от лица пользователя.
Примеры: "Расскажи подробнее про этот сорт", "А если я живу в северном регионе?", "Можешь составить график полива?".
...
Верните ответ СТРОГО в формате JSON:
{
  ...
  "suggested_actions": ["Уточняющий вопрос 1", "Уточняющий вопрос 2"],
  "disclaimer": "Рекомендации могут быть неточными."
}
"""
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[KeyboardButton(q)]
                for q in result.get('suggested_actions', [])[:3]]
