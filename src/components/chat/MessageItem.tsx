    ...
    >
      <div className={`max-w-[85%] sm:max-w-[75%] ${isAssistant ? 'w-full' : ''}`}>
        ...
        >
          ...
          {isAssistant && message.data && (message.data.possible_causes.length > 0 || message.data.recommendations.length > 0) && (
            <div className="mt-4 space-y-4 pt-4 border-t border-brand-accent/20">
              ...
              {message.data.recommendations.length > 0 && (
                <div className="p-3 bg-brand-light rounded-2xl border border-brand-accent/30">
                  <h4 className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-brand-dark mb-2">
                  ...
                  <ul className="space-y-1">
                    {message.data.recommendations.map((rec, i) => (
                      <li key={i} className="text-sm flex items-start gap-2 text-gray-700">
                        <CheckCircle2 className="w-3 h-3 text-brand-medium mt-0.5 shrink-0" />
                        {rec}
                      </li>
                    ))}
                  </ul>
